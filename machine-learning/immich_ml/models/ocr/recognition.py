...
class TextRecognizer(InferenceModel):
    ...
    def __init__(self, model_name: str, min_score: float = 0.9, **model_kwargs: Any) -> None:
        ...
        self.input_name = "x"
        self.rknn_runner: RknnTextRecognitionRunner | None = None
    ...
    def _load(self) -> ModelSession:
        if self.model_format == ModelFormat.RKNN:
            ...
            self.rknn_runner = RknnTextRecognitionRunner(
    ...
    def _predict(self, img: Image.Image, texts: TextDetectionOutput) -> TextRecognitionOutput:
        ...
        if self.model_format == ModelFormat.RKNN:
            if self.rknn_runner is None:
                raise RuntimeError("RKNN OCR recognition runner is not initialized")
            rec_texts, rec_scores = self.rknn_runner.predict(
                session=self.session,
                input_name=self.input_name,
                images=self.get_crop_img_list(img, boxes),
            )
